...
public class SimpleComboBox<T> extends ComboBox<SimpleComboValue<T>> {
  ...
  public void add(T value) {
    store.add(new SimpleComboValue(value));
  }
  ...
  public void remove(T value) {
    for (SimpleComboValue c : store.getModels()) {
  ...
  public void setSimpleValue(T value) {
    SimpleComboValue c = findModel(value);
    if (c != null) {
      setValue(c);
    }
  }
   ...
   */
  public SimpleComboValue findModel(T value) {
    SimpleComboValue val = null;
    for (SimpleComboValue c : store.getModels()) {
      if (c.getValue().equals(value)) {
        val = c;
        break;
